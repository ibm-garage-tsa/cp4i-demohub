...
<AnchorLinks>
  ...
  <AnchorLink>Access your ROKS cluster by Command Line</AnchorLink>
</AnchorLinks>
...
4.  Select the Redhat Openshift Cluster that you just created from the list. The DTE provisioned cluster name starts with dteroks or playgrowth.

    ![](images/roks-cluster.png)

5.  Click **Add project** Enter a project name, and a tag name to identify your cluster.

    ![](images/add-project.png)

6.  Enter **cp4i** as Project name and click **Add**.

    ![](images/cp4i-project.png)

7.  Scroll down. On *Set the deployment values* section. Unmark the **Use an existing secret?** to define a new password and enter a value for csDefaultAdminPassword. It is important that you choose a password that is 32 characters in length and it conforms to the password rule - number, letter, -.
...
8.  Scroll down to 'Parameters with default values' and click on the 'twisty' to open them. There is no need to change the default values except for the parameters that start with demo.
...
9.  Accept the license agreement and click **Install** to install Cloud
...
11.  When you see the **Active** status, you will be able to access Cloud Pak for Integration Platform Navigator by selecting **Offering Dashboard** on the upper right corner of the page.
...
## Access your ROKS cluster by Command Line

Some demos and labs requires access to ROKS cluster by command line. Follow the steps below to log in your cluster.

1.  Back to the OpenShift Managed Cluster Web Console (if you are not there, you should have an email with a link to OpenShift Console, you received this e-mail when you requested the ROKS environment).

    ![](images/roks-console.png)

2.  On the top right of the screen, click on your **username** and select **Copy Login Command**.

    ![](images/copy-login-cmd.png)
...
4.  Copy the **log in** command.
    ...
    ![](images/login-token.png)

5.  On a terminal window, **paste** and **run** the log in command.
